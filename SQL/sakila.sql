...
-- Use this SQL Querry to read the actor_id
SELECT 
    actor_id
FROM
    actor
...
-- Use actor_id from previous output to update the table in safe more
UPDATE actor 
SET 
    first_name = 'HARPO'
WHERE
...
SELECT 
    f.title AS 'Title', COUNT(fa.actor_id) AS 'No. of Actors'
FROM
    film f
        INNER JOIN
...
SELECT 
    COUNT(i.inventory_id) AS 'Total Count'
FROM
    film f
        INNER JOIN
...
SELECT 
    title, COUNT(f.film_id) AS Count
FROM
    film f
        INNER JOIN
    inventory i ON (f.film_id = i.film_id)
...
SELECT 
    s.store_id, SUM(amount) AS Business
FROM
    store s
        INNER JOIN
...
SELECT 
    s.store_id AS StoreID, c.city AS City, co.country AS Country
FROM
    store s
        LEFT JOIN
    address a ON s.address_id = a.address_id
...
-- select * from rental;
SELECT 
    c.name, SUM(p.amount) AS Gross_Amount
FROM
    category c
        INNER JOIN
